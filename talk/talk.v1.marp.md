...
graph TD
    C["🗄️ Catalog (DB)"] --> M["📋 metadata.json"]
    M --> ML["📦 snap-*.avro"]
    ML --> MF1["📦 manifest-1.avro"]
    ML --> MF2["📦 manifest-2.avro"]
    MF1 --> D1["🗃️ data-001.parquet"]
    MF1 --> D2["🗃️ data-002.parquet"]
    MF2 --> D3["🗃️ data-003.parquet"]
    MF2 --> D4["🗃️ data-004.parquet"]
</div>
...
1. **Metadata Traversal**: Читаем Snapshot $\to$ Manifest List $\to$ Manifests.
...
1. **Data Phase**: Пишем данные, собираем статистику.
2. **Metadata Phase**: Формируем дерево метаданных.
3. **Commit Phase**: Linearizable операция в каталоге.
...
<div class="mermaid">
flowchart LR
    Start((Start)) --> Read[Read Base v1]
    Read --> Write[Write Data]
    Write --> Check{v1 == Current?}
    Check -- Yes --> Commit[Commit v2]
    Check -- No --> Rebase[Rebase on v2]
    Rebase --> Check
</div>
...
* **Safe Type Promotion**: `int` $\to$ `long`, `float` $\to$ `double`.
...
1. **Metadata Tree**: `Snapshot` $\to$ `Manifest List` $\to$ `Manifest` $\to$ `Data`.
